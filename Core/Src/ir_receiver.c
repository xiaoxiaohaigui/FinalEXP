 ...
 * @file           : ir_receiver.c
 * @brief          : NEC 红外接收驱动 — 实现
 * @description    : 使用 TIM7（CubeMX 配置的 1MHz/1µs 时基）和 PG0 上的
...
/**
 * @brief  安全读取 TIM7 自由运行计数器的当前值。
 * @note   TIM7 由 CubeMX 配置：72MHz / 72 = 1MHz，即 1 tick = 1µs。
 */
 ...
 */
static uint32_t
IR_Duration(uint32_t now, uint32_t prev)
{
    if(now >= prev)
    {
        return now - prev;
...
IR_Time_In_Range(uint32_t val, uint32_t min, uint32_t max)
{
...
static const IR_Key_Entry_t ir_key_table[] = {
    { 0x43, "PLAY/PAUSE" }, { 0x44, "PREV" }, { 0x40, "NEXT" }, { 0x45, "CH-" }, { 0x46, "CH" },
    { 0x47, "CH+" },        { 0x07, "VOL-" }, { 0x15, "VOL+" }, { 0x09, "EQ" },  { 0x16, "0" },
    { 0x0C, "1" },          { 0x18, "2" },    { 0x5E, "3" },    { 0x08, "4" },   { 0x1C, "5" },
    { 0x5A, "6" },          { 0x42, "7" },    { 0x52, "8" },    { 0x4A, "9" },   { 0x19, "100+" },
    { 0x0D, "200+" },
};
 ...
 */
const char *
IR_Get_Key_Name(uint8_t command)
{
 ...
 *         TIM7 已由 CubeMX 在 tim.c 中配置。
 *         调用本函数前必须先调用 Delay_Init() 启动 TIM7。
 */
void
IR_Receiver_Init(void)
{
    /* 以干净的状态开始 */
    IR_Reset_State();
}
 ...
 */
void
IR_EXTI_Callback(uint16_t GPIO_Pin)
{
...
uint8_t
IR_Is_Data_Ready(void)
{
    return ir_data_ready;
}

void
IR_Clear_Data_Ready(void)
{
    ir_data_ready = 0;
    ir_is_repeat = 0;
    IR_Reset_State();
 ...
 */
uint16_t
IR_Read_Data(char *buf, uint16_t size)
{
    if(buf == NULL || size == 0)
    {
        return 0;
    ...
    if(ir_is_repeat)
    {
        ...
        return (uint16_t)snprintf(
          buf, size, "[IR] REPEAT  Key=%s\r\n", IR_Get_Key_Name(ir_last_command));
    }
    else
    {
 ...
 */
void
IR_Send_Data_Over_UART(const uint8_t *data, uint16_t len)
{
    if(data == NULL || len == 0)
    {
        return;
